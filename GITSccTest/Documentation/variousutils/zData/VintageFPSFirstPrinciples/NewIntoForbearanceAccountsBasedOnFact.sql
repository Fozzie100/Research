...
select * from loader.JobDefinition where JobDefinitionCode like '%BPF_VNTG_ARRSLVLGRIP_BOOK%'
select * from Phil.FPAccountsBI where AnchorDate='31 Mar 2016' order by AccountNumber
select * from Phil.ForbearanceAccountsStageExtra where COBDate='31 Mar 2016' order by AccountNumber


select count(*) from Phil.FPAccountsBI  where AnchorDate='31 Mar 2016'

select count(*) from Phil.FPAccountsChargeOffBI where AnchorDate='31 Mar 2016'

select count(*) from Phil.ChargeOffAccountsBI where AnchorDate='31 Mar 2016'

select count(*) from Phil.ChargeOffStageExtra where COBDate='31 Mar 2016'
select count(*) from Phil.ForbearanceAccountsStageExtra where COBDate='31 Mar 2016'

select * from bi.vwGRCR_BPF_Vintage_FPs_PastDue_MOB_Aggregate
where AnchorDate='201603'
 ...
 CREATE TABLE #NewIntoForbearance
            ...
            WHERE      base.netbalance_finance IS NOT NULL 
				...
				select   count(*) -- top 100 x.*, j.CobDate
				 from [stg].[N9DEBTP] x
				inner join loader.JobRun jr
				on jr.Run = x.Run
				inner join loader.Job j
				on j.JobId = jr.JobId
				where j.CobDate = '31 Dec 2015'