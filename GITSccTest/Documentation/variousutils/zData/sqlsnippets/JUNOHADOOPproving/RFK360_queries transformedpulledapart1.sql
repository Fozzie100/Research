...
--QUERY 2
SELECT --last_day(Add_months(business_date.business_date, -1)),
	EOMONTH(DATEADD(month, -1, business_date.business_date)),
	party.customer_system_reference,
	party_rating.code,
	...
	Account_Industry_BIC.lvl1_name
FROM business_date 
INNER JOIN account_risk_fact ON (account_risk_fact.cob_date_key=business_date.[key])
INNER JOIN account ON (account.[key]=account_risk_fact.account_key)
INNER JOIN v_industry_bic  Account_Industry_BIC ON (account.industry_key=Account_Industry_BIC.lvl4_key)
INNER JOIN account_team_relationship ON (account.[key]=account_team_relationship.account_key)
INNER JOIN v_team_cth  Table__8 ON (account_team_relationship.team_key=Table__8.lvl4_key)
INNER JOIN party ON (party.[key]=account.counterparty_key)
INNER JOIN party_rating_relationship ON (party.[key]=party_rating_relationship.party_key)
INNER JOIN party_rating ON (party_rating_relationship.rating_key=party_rating.[key])
INNER JOIN party_rating_role ON (party_rating_relationship.role_key=party_rating_role.[key])
WHERE party_rating_role.code  IN  ( 'DG_PIT'  )
AND account_team_relationship.role_code  =  'CRP'
--AND business_date.business_date >=to_date(last_day(Add_months(trunc({d '2015-09-30'} ,'month') ,-14))) 
AND business_date.business_date >= EOMONTH(DATEADD(month, -14, '2015-09-30'))
--and business_date.business_date <=to_date(last_day(Add_months(trunc({d '2015-09-30'} ,'month') ,-1)))  )
AND business_date.business_date <= EOMONTH(DATEADD(month, -1, '2015-09-30'))
AND party_rating.code  NOT IN  ( '23','0.1','22'  )
AND Table__8.lvl1_name IN  ( 'Larger Business SBU','Medium Business and Agriculture SBU','Specialist Products SBU','Scotland and Ireland SBU'  )
GROUP BY --last_day(Add_months(business_date.business_date, -1)), 
	EOMONTH(DATEADD(month, -1, business_date.business_date)),
	party.customer_system_reference, 
	party_rating.code, 
	business_date.business_date, 
